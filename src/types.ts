...
export type LinuxEvdevHelperConfig = {
  parentPid: number;
  devices: string[];
  hotkeys: Array<LinuxEvdevHotkey & { parsed: LinuxEvdevParsedHotkey }>;
  enableUinput: false;
};

export type LinuxEvdevHelperCommand =
  | { type: "bind"; hotkey: LinuxEvdevHotkey & { parsed: LinuxEvdevParsedHotkey } }
  | { type: "unbind"; id: string }
  | { type: "clear" }
  | { type: "set"; hotkeys: Array<LinuxEvdevHotkey & { parsed: LinuxEvdevParsedHotkey }> };
