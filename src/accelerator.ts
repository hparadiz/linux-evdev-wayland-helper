...
const modifierAliases = new Map<string, LinuxEvdevParsedHotkey["modifiers"][number]>([
  ["ctrl", "ctrl"],
  ["control", "ctrl"],
  ["cmdorctrl", "ctrl"],
...
function isBasicAlphabetKey(keyCode: string): boolean {
  return /^KEY_[A-Z]$/.test(keyCode);
}

export function parseAccelerator(accelerator: string): LinuxEvdevParsedHotkey {
  ...
  const modifiers = new Set<LinuxEvdevParsedHotkey["modifiers"][number]>();
